...
class TestLogin(ViewTestMixin):
    ...
    def test_login(self):
        ...
        response = self.login()
        assert response.status_code == 200
        assert_status_with_message(200, response, 'Pick a Username')
...
class TestSignup(ViewTestMixin):
    ...
    def test_begin_register_fail_logged_in(self, users):
        """ Register should redirect to settings. """
        self.login()
        response = self.client.get(url_for('user.register'),
                                   follow_redirects=False)
    ...
    def test_begin_register_fail(self):
        """ Register failure due to using an account that exists. """
        user = {'email': 'admin@local.host', 'password': 'password'}
        response = self.client.post(url_for('user.register'), data=user,
                                    follow_redirects=True)

        assert_status_with_message(200, response, 'Already exists.')

    def test_register(self, users):
...
class TestUpdateCredentials(ViewTestMixin):
    ...
    #     # user = User(email='disabled@local.host', password='password')
    #     response = self.client.post(url_for('user.update_credentials'),
    #                                 data=user, follow_redirects=True)
    #     pytest.set_trace()
    #     assert_status_with_message(200, response, 'Already exists.')
    ...
    def test_begin_update_credentials_email_change(self):
        ...
        new_user = User.find_by_identity('admin_update@local.host')
